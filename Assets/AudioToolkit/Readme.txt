==============================================================
  Audio Toolkit v12.0 - (c) 2019 by ClockStone Software GmbH
==============================================================
...
v10.7
- fix a bug where new AudioItems added by code to additional AudioControllers
  could not be found.
- fix a bug in the pooling system
- sequence of spatial audio items is played at correct position

v11.0  WARNING: upgrade requires code changes
- new feature: AudioChannels (custom channels in addition to Music, Ambience)
- new feature: PlayInSync()
- high precision time methods (timeUntilEnd now double and considers pitch), pcsTime, pcmSampleTime) 
improve audio syncing with PlayAfter or PlayInSync
- play list per channel (instead of only for Music)
- moved all scripts to namespace CS.AudioToolkit
- refactored many methods and properties (needs some backwards compatibility changes)

==============
Upgrade Infos:
==============
- change "using ClockStone" to "using CS.AudioToolkit"
- use AudioController.GetChannel(AudioChannelType) for various functions
  like musicEnabled, GetCurrentMusic(), IsMusicPaused(), etc
- use SetCurrentPlaylist(...) instead of SetCurrentMusicPlaylist
- use JumpToNextOnPlaylist() instead of PlayNextMusicOnPlaylist() (same with Previous)

v11.1
- fix setting category volume of playing audios when the objects AudioController was already destroyed 
  (e.g. due to scene change)
- improve Audio Log View
- Pooling classes: fixes + new option: sendAwakeToInactive
- fix for latest Unity versions (2021.3 and 2022.3, 2023, 6)

v11.2
- new feature: "Play with zero volume" setting override in AudioCategory
- bugfix: Disabled and stopped audio objects do not restart playing anymore when enabled again
- new method: AudioController.GetCurrentPlaylistAudioObject(...)
- improvement: if an AudioObject gets stopped and disabled in the same frame it will now be destroyed or 
  moved to the pool correctly
- pooling system update: 
   * Unity 2023+: Awake/Start Messages are now always sent to inactive objects
   * Destroy Object (non-Immediate): Moving objects to the pool is now postponed to a later time in same 
     frame to avoid the problem that objects can not be reparented during certain Unity Callbacks such as 
     OnDisable
   * Awake/Start/OnDestroy is now always called by reflection to allow sending messages to inactive objects

v12.0
- Pooling system rework (execution order of Awake, OnEnable and Start improved to match Unity more closely)